      ...
      <div class="blog-main">
         ...
         <div class="blog-body">
            ...
            <section id="introduction">
               ...
               <p class="caption">
                  <vm :formula='`$x_t$`' /> represents the input sequence, <vm :formula='`$h_t$`' /> the hidden states,
                  <vm :formula='`$y_t$`' /> the prediction, and <vm :formula='`$L_t$`' />, the individual loss.
               </p>
               <p>
                  ...
                  <vm :formula='`$f_h$`' /> requires, as arguments, it's corresponding input <vm :formula='`$x_t$`' /> and prior hidden state
                  <vm :formula='`$h_{t - 1}$`' /> to produce the next hidden state <vm :formula='`$h_t$`' />. <u>Two:</u> The gradients in backpropagation will be summed at each
                  ...
                  This is one of a <i>"handful of different ways"</i> to represent the input and output for an RNN. <u>Four:</u> a inital hidden state, <vm :formula='`$h_0$`' />, must be provided for the forward pass
                  of an RNN. The initial hidden state is either learnt (the output of  network <i>x</i> can be used to to populate <vm :formula='`$h_0$`' /> in RNN <i>y</i> ) or set to 0.
            ...
            <section id="sequenceprocessing">
               ...
               <p>
                  ...
                  there are different ways of processing the data. For the many to many shown above, consider a video as input where the frames of the video compose the sequence. <vm :formula='`$x_1$`' /> would be the first frame,
                  <vm :formula='`$x_2$`' /> would be the second... and so on. For this type of many-to-many, our output at each timestep could then be some decision/classification
            ...
            <section id="forward">
               ...
               <p>
                  As mentioned above, a characteristic to RNNs is that weights are shared temporally, between all time steps. A simple Vanilla RNN can have three weight tensors: <vm :formula='`$W_{hh}$`' />, <vm :formula='`$W_{xh}$`' />,
                  <vm :formula='`$W_{hy}$`' /> and only a couple of bias parameters <vm :formula='`$b_h$`' /> and <vm :formula='`$b_y$`' />. Each of these parameters
                  are recycled at each hidden step to compute either a local prediction or the next hidden step.
                  It helps me to think of the index of tensors as <vm :formula='`$W_{(from)\\;(to)}$`' />
               ...
               <section id="forwardhidden">
                  ...
                  <p>
                     Transformation between hidden steps <vm :formula='`$h_{t-1} \\rightarrow h_t$`' />: 
                     <vm class="math" :formula='this.latex.rnnStep' />
                  </p>
               ...
               <section id="forwardpred">
                  ...
                  <p>
                     Transformation between hidden steps & prediction <vm :formula='`$h_{t} \\rightarrow \\hat{y}$`' />:
                     <vm class="math" :formula='this.latex.yhatTransform' />
                  </p>
            ...
            <section id="onestepback">
               ...
               <p>
                  Here are the partials we're looking for: 
               </p>
               <div class="math-b">
                  <vm class="math" :formula='`$$\\frac{\\partial{L}}{\\partial{W_{hy}}},
               ...
               <div class="math-b">
                  <vm class="math" :formula='`$$\\frac{\\partial{L}}{\\partial{W_{hy}}} = \\frac{\\partial{L}}{\\partial{\\hat{y_t}}}\\cdot\\frac{\\partial{\\hat{y_t}}}{\\partial{W_{hy}}}=output\\_grad\\;\\cdot h_t^\\top$$`' />
                  <br>
                  <vm class="math" :formula='`$$\\frac{\\partial{L}}{\\partial{b_y}} = \\frac{\\partial{L}}{\\partial{\\hat{y_t}}}\\cdot\\frac{\\partial{\\hat{y_t}}}{\\partial{b_y}}=\\sum^N{output\\_grad}$$`' />
                  <br>
                  <vm class="math" :formula='`$$\\frac{\\partial{L}}{\\partial{h_t}} = \\frac{\\partial{L}}{\\partial{\\hat{y_t}}}\\cdot\\frac{\\partial{\\hat{y_t}}}{\\partial{h_t}}=output\\_grad\\;\\cdot W_{hy}^\\top$$`' />
               </div>
               <p>
                  Everything beyond <vm :formula='`$h_t$`' /> runs through the element-wise <i>tanh</i> non-linearity so I create an intermediary variable which I will call <vm :formula='`$dtanh$`' /> that uses a hyperbolic identity:
                  <vm :formula='`$\\frac{\\mathrm{d}tanh(x)}{\\mathrm{d}} = sech^2(x) = 1-tanh^2(x)$`' /> to simplify finding the derivative since <vm :formula='`$tanh(x)$`' /> is already provided to us in from the forward pass.
               ...
               <div class="math-b">
                  <vm class="math" :formula='`$$\\frac{\\partial{L}}{\\partial{W_{hh}}} = \\frac{\\partial{L}}{\\partial{\\hat{y_t}}}\\cdot\\frac{\\partial{\\hat{y_t}}}{\\partial{h_t}}\\cdot\\frac{\\partial{h_t}}{\\partial{W_{hh}}}=dtanh\\;\\cdot h_{t-1}^\\top$$`' />
                  <br>
                  <vm class="math" :formula='`$$\\frac{\\partial{L}}{\\partial{h_{t-1}}} = \\frac{\\partial{L}}{\\partial{\\hat{y_t}}}\\cdot\\frac{\\partial{\\hat{y_t}}}{\\partial{h_t}}\\cdot\\frac{\\partial{h_t}}{\\partial{h_{t-1}}}=dtanh\\;\\cdot W_{hh}^\\top$$`' />
                  <br>
                  <vm class="math" :formula='`$$\\frac{\\partial{L}}{\\partial{b_h}} = \\frac{\\partial{L}}{\\partial{\\hat{y_t}}}\\cdot\\frac{\\partial{\\hat{y_t}}}{\\partial{h_t}}\\cdot\\frac{\\partial{h_t}}{\\partial{h_{t-1}}}=\\sum^N{dtanh}$$`' />
                  <br>
                  <vm class="math" :formula='`$$\\frac{\\partial{L}}{\\partial{W_{xh}}} = \\frac{\\partial{L}}{\\partial{\\hat{y_t}}}\\cdot\\frac{\\partial{\\hat{y_t}}}{\\partial{h_t}}\\cdot\\frac{\\partial{h_t}}{\\partial{W_{xh}}}=dtanh\\;\\cdot x_t^\\top$$`' />
               </div>
            ...
            <section id="fullback">
               ...
               <p>
                  Second: As we proceed to the next hidden state during backpropagation, there can be a "redundancy" of computed gradients. To be more concrete, lets say we've computed the gradients at time step <vm :formula='`$h_3$`' />
                  of a many to many styled RNN.
                  As we proceed to find the gradients at time step <vm :formula='`$h_2$`' />, the gradient <vm :formula='`$\\frac{\\partial{L}}{\\partial{h_{t-1}}}$`' /> from time step <vm :formula='`$h_3$`' />
                  overlaps with <vm :formula='`$\\frac{\\partial{L}}{\\partial{h_{t}}}$`' /> at time step <vm :formula='`$h_2$`' />. This isn't a problem, just food for thought. Similar to the resolution in the first
               ...
               <p>
                  ...
                  The second is a series of summing two overlapping, but different, gradients. Even though <vm :formula='`$\\frac{\\partial{L}}{\\partial{h_{t-1}}}$`' /> and <vm :formula='`$\\frac{\\partial{L}}{\\partial{h_{t}}}$`' />
                  calculate the gradient wrt to the same variable between two backward steps, <vm :formula='`$\\frac{\\partial{L}}{\\partial{h_{t-1}}}$`' /> computes it's value from all upstream gradients
                  through <vm :formula='`$f_h$`' /> while <vm :formula='`$\\frac{\\partial{L}}{\\partial{h_{t}}}$`' /> computes it's value from the local loss through 
                  <vm :formula='`$f_y$`' />.
               </p>
            ...
            <section id="thoughts">
               <h2>Thoughts</h2>
                 <p>
                  ...
                  seem to have different information on them. Some people use a linear transformation between <vm :formula='`$h_t$`' /> and <vm :formula='`$\\hat{y}_t$`' /> as I did, 
